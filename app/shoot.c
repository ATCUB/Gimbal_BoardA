...
void shoot_init(void)
{
    ...
    ramp_init(&shoot_control.fric1_ramp, SHOOT_CONTROL_TIME * 0.001f, FRIC_UP_MAX, FRIC_OFF);
    ramp_init(&shoot_control.fric2_ramp, SHOOT_CONTROL_TIME * 0.001f, FRIC_UP_MAX, FRIC_OFF);
...
static void shoot_feedback_update(void)
{
    ...
    shoot_control.angle = (shoot_control.ecd_count * ECD_RANGE + shoot_control.shoot_motor_measure->ecd) * MOTOR_ECD_TO_ANGLE;
		//将输出轴角度规整到+-PI
		if(shoot_control.ecd_count > 0)
			shoot_control.angle -= PI;
		else if(shoot_control.ecd_count < 0)
			shoot_control.angle += PI;
    //微动开关
